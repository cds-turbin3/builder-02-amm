...
- [Test scenarios in two views](#test-scenarios-in-two-views)
- [Running](#running)
- [See also](#see-also)
...
Four representative tests, each rendered two ways: the structured CPI tree (visible, from `print_logs_structured`) and a Mermaid sequence diagram with lifelines (collapsed in a `<details>`, from `print_mermaid_with_lifelines`). Both views come from the same captured transaction; the `print_markdown_pair()` method on `TransactionResult` (in `anchor-litesvm` on `feat/euler`) uses `.tap(...)` to interleave the markdown delimiters so that `cargo test --nocapture` produces output you can paste directly into a markdown file. The scenarios below were captured exactly that way.

**These captures are illustrative historical snapshots.** They predate the move to deterministic keypairs, so their addresses (and exact compute-unit numbers) belong to those old random runs; reason from the trees' *shape*, not the literal values. The canonical, deterministic, regenerable view is the committed [`docs/testing/test-report.md`](docs/testing/test-report.md) (one before/after section per scenario, regenerated by `just test-md`). See [`anchor-litesvm`'s `docs/CONVENTIONS.md`](https://github.com/cds-rs/anchor-litesvm/blob/feat/euler/docs/CONVENTIONS.md) for the house standard these tests follow.
